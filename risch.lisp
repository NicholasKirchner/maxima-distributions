...
(declare-top (special prob rootfac parnumer pardenom logptdx wholepart
                      ...
                      context bigfloatzero rp-polylogp *in-risch-p* distint))
...
(defun rischint (exp intvar &aux ($logarc nil) ($exponentialize nil)
  ...
  (prog ($%e_to_numlog $logsimp trigint operator y z var ratform liflag
     ...
     (rischform exp)
     (cond (distint (return (distribint exp intvar)))
           (trigint (return (trigin1 exp intvar)))
  ...
  (cond ((or (atom l) (alike1 intvar l) (freeof intvar l)) nil)
        ...
        ((atom (caar l))
         (case (caar l)
           ...
           ((%heaviside %delta %doublet)
            (setq distint t)
            (rischform (cadr l)))
           (t (setq operator (caar l)))))
        (t (setq operator (caar l)))))

(defun remult (begins ends)
  ;;;Support function to distribint for remultiplying
  (let ((factors (append begins ends)))
    (cond ((null factors) 1)
          ((cdr factors) (cons '(mtimes simp) factors))
          (t (car factors)))))

(defun distribint (exp var)
  ;;;Hack to handle exclusively int(distrib*f(x),x), where distrib can
  ;;;be heaviside, delta, or doublet with a linear argument.
  ;;;No other cases handled, except distributing over addition.
  (setq distint nil)
  (let (ab firstfactors lastfactors thisfactor offset op antideriv sign hresult)
    (cond ((eq (caar exp) 'mtimes)
           (setq lastfactors (cdr exp))
           (loop
            (cond (lastfactors
                   (setq thisfactor (car lastfactors))
                   (setq lastfactors (cdr lastfactors))
                   (setq firstfactors (append (list thisfactor) firstfactors))
                   (cond ((or (atom thisfactor) (alike1 var exp)
                              (freeof var exp)) nil)
                         ((member (caar thisfactor)
                                  '(%heaviside %delta %doublet))
                          (setq thisfactor (simplifya thisfactor nil))
                          (setq ab (islinear (cadr thisfactor) var))
                          (cond (ab
                                 (setq offset (div (cdr ab) (car ab)))
                                 (setq op (caar thisfactor))
                                 (setq exp (remult lastfactors
                                                   (cdr firstfactors)))
                                 (return-from distribint (simplifya
                                          (cond ((eq op '%heaviside)
                                                 (setq antideriv (rischint exp var))
                                                 (setq sign (asksign (car ab)))
                                                 (setq hresult
                                                       (list '(mtimes)
                                                             (list '(%heaviside)
                                                                   (list '(mplus)
                                                             ...
                                                             (list '(mplus)
                                                                   antideriv
                                                                   (list '(mtimes)
                                                                         (maxima-substitute (neg offset) var antideriv) -1))))
                                                 ...
                                                 (list '(mtimes)
                                                       ...
                                                       (list '(%heaviside)
                                                             (list '(mplus)
                                                                   var
                                                                   offset))
                                                       (maxima-substitute (neg offset) var exp)))
                                                ((eq op '%doublet)
                                                 (list '(mtimes)
                                                       -1
                                                       (list '(mexpt)
                                                        ...
                                                        (list '(mtimes)
                                                         (list '(%delta)
                                                               (list '(mplus)
                                                                     var
                                                                     offset))
                                                         (maxima-substitute (neg offset) var exp)))))) nil)))
                                (t nil)))
                         (t nil)))
                  (t (return-from distribint (list '(%integrate) exp var))))))
          ((eq (caar exp) 'mplus)
           (simplus (cons '(mplus) (mapcar #'(lambda (x) (rischint x var))
                                           (cdr exp))) 1 t))
          ((eq (caar exp) '%heaviside)
           (setq ab (islinear (cadr exp) var))
           (cond (ab
                  (setq offset (div (cdr ab) (car ab)))
                  (setq sign (asksign (car ab)))
                  (setq hresult
                        (list '(mtimes)
                              (list '(mplus) var offset)
                              (list '(%heaviside)
                                    (list '(mplus) var offset))))
                  (cond ((eq sign '$positive) hresult)
                        (t
                         (list '(mplus)
                               var
                               (list '(mtimes) hresult -1)))))
                 (t
